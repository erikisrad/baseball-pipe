...
async def serve_broadcast(request):
    ...
    if error:
        ...
        broadcast_html = gen_broadcast_html(selected_broadcast, video_url)
        downloads_html = await gen_downloads_html(stream, video_url)
...
def gen_broadcast_html(selected_broadcast, video_url):
...
async def gen_downloads_html(stream, video_url):
    ...
    <a class="dl" href="{await stream.get_master_playlist_url()}" download>Master Playlist</a>'''
    ...
    if variants:
        for variant in variants:
            ...
            variant_url = urljoin(await stream.get_upstream_base_url(), variant.uri)
