    ...
    async def _post_identity(self):
        ...
        async with self.session.post(IDENTITY_URL, headers=headers, data=payload, proxy=self.proxy) as res:
            ...
            if not self._id_email or not self._id_password:
                ...
                raise Exception(f"IDENTITY failed, email or password authenticator not found in IDENTITY response: {res_text}")
    ...
    async def _answer(self):
        ...
        async with self.session.post(ANSWER_URL, headers=headers, data=payload, proxy=self.proxy) as res:
            ...
            if not self._interaction_code:
                res_text = await res.text()
                raise Exception(f"ANSWER failed: interaction code not found in response: {res_text}")
